...
int categorize(int signal){
  int cat;
  if( signal == 48 ) cat = 0;
  else if( signal == 40) cat = 1;
  else if( signal == 36) cat = 2;
  else if( signal == 34) cat = 3;
  else if( signal == 33) cat = 4;
  else if( signal == 24) cat = 5;
  else if( signal == 20) cat = 6;
  else if( signal == 18) cat = 7;
  else if( signal == 17) cat = 8;
  else if( signal == 12) cat = 9;
  else if( signal == 10) cat = 10;
  else if( signal == 9) cat = 11;
  else if( signal == 6) cat = 12;
  else if( signal == 5) cat = 13;
  else if( signal == 3) cat = 14;
  else cat = 15;
  return cat;
}
...
//  }
  //LorentsVector
  TLorentzVector jet1, jet2, jet3, jet4, jet5, jet6, addbjet1, addbjet2;
  ...
  jet5.SetPtEtaPhiE(jet_pt[sortjet_idx[4]], jet_eta[sortjet_idx[4]], jet_phi[sortjet_idx[4]], jet_e[sortjet_idx[4]]);
  jet6.SetPtEtaPhiE(jet_pt[sortjet_idx[5]], jet_eta[sortjet_idx[5]], jet_phi[sortjet_idx[5]], jet_e[sortjet_idx[5]]);
  ...
  if( jet5.DeltaR(addbjet1) < 0.4 || jet5.DeltaR(addbjet2) < 0.4){
      signal = signal + 2;
  }
  if( jet6.DeltaR(addbjet1) < 0.4 || jet6.DeltaR(addbjet2) < 0.4){
      signal = signal + 1;
  }
...
void ana_ttbb_j6(){
  ...
  //nJets >= 6
  auto df_goodjet = df_goodlepton.Filter("ngoodjets >=6 ", "Events with at least 6 goodjets");
...
////  df_cat6.Snapshot("catetory_6", "ttbb_invmasstest.root", {"category", "jet1_pt", "jet1_eta", "jet1_btag", "jet1_e", "jet2_pt", "jet2_eta", "jet2_btag", "jet2_e", "jet3_pt", "jet3_eta", "jet3_btag", "jet3_e", "jet4_pt", "jet4_eta", "jet4_btag", "jet4_e", "deltaR_12", "deltaR_13", "deltaR_14", "deltaR_23", "deltaR_24", "deltaR_34", "lepton_pt", "lepton_eta", "lepton_e", "deltaR_lepj1", "deltaR_lepj2", "deltaR_lepj3", "deltaR_lepj4"}, opts);
  ...
  df_bjet.Snapshot("dnn_input", "ttbb_j6_test.root", {"signal", "category"});
